...
function CategoryPage() {
  ...
  const models = useMemo(() => {
    ...
    return list;
  }, [c.models, sort, maxPrice, maxSweep, maxPower]);
    ...
    <div className="py-16 px-4 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
        <nav className="flex items-center gap-1 text-xs font-[Inter] text-slate-500">
          ...
          <ArrowRight className="h-3 w-3" />
          <span className="text-[#0d4361] font-semibold">{c.name}</span>
        </nav>
        ...
        <div className="mt-6 grid lg:grid-cols-2 gap-10 items-center">
          <div>
            <span className="inline-block font-[Inter] text-xs font-bold tracking-[0.2em] uppercase text-[#0d6b78]">{c.tagline}</span>
            <h1 className="mt-3 font-[Poppins] text-4xl sm:text-5xl font-extrabold text-[#0a2f44]">{c.name}</h1>
            <p className="mt-5 font-[Inter] text-slate-600 leading-relaxed">{c.description}</p>
            <button
              type="button"
              onClick={async () => {
        ...
        <div className="mt-6 grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {models.map((mm: Model) => (
            <Link
              ...
              params={{ category: c.slug, model: mm.slug }}
        ...
        <div className="mt-16">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {categories.filter((cc) => cc.slug !== c.slug).map((cc) => (
              <Link key={cc.slug} to="/products/$category" params={{ category: cc.slug }} className="rounded-full bg-white/70 ring-1 ring-[#0d6b78]/20 px-4 py-2 font-[Inter] text-sm text-[#0d4361] hover:bg-[#0d6b78]/10 transition-colors">
                {cc.name}
              </Link>
            ))}
          </div>
