...
function collectSpecs(model) {
	...
	if (model.specifications?.length) {
		for (const s of model.specifications) if (s.value && !rows.find(([k]) => k.toLowerCase() === s.label.toLowerCase())) rows.push([s.label, s.value]);
	}
	return rows;
}
async function drawLogo(doc, x, y, maxH) {
	const img = await loadImage(csi_logo_default);
	if (!img) return 0;
...
async function drawHeader(doc, title, subtitle) {
	...
	await drawLogo(doc, 12, 5.5, pillH - 3);
	doc.setTextColor(255, 255, 255);
	doc.setFont("helvetica", "bold");
	doc.setFontSize(11);
	doc.text(title, 50, 12);
	doc.setFont("helvetica", "normal");
	doc.setFontSize(8);
	doc.setTextColor(220, 235, 245);
	doc.text(subtitle, 50, 17);
	doc.setFontSize(8);
	doc.setTextColor(220, 235, 245);
...
async function drawModelPage(doc, model, cat) {
	const w = doc.internal.pageSize.getWidth();
	const h = doc.internal.pageSize.getHeight();
	const contentTop = 30;
	const margin = 14;
	...
	doc.setFont("helvetica", "bold");
	doc.setFontSize(8);
	doc.text(cat.name.toUpperCase(), 18, 34.2);
	doc.setTextColor(TEXT.r, TEXT.g, TEXT.b);
	doc.setFont("helvetica", "bold");
	doc.setFontSize(22);
	doc.text(model.name, margin, 46);
	doc.setFont("helvetica", "normal");
	doc.setFontSize(10);
	doc.setTextColor(ACCENT.r, ACCENT.g, ACCENT.b);
	doc.text(`Model No. ${model.modelNo}`, margin, 52);
	const imgY = 58;
	...
	if (model.highlights?.length) {
		...
		for (const hl of model.highlights) {
			...
			if (col === 1) rowY += Math.max(lines.length, 1) * 4.4 + 1;
			col = col === 0 ? 1 : 0;
		}
		if (col === 1) rowY += 4.4;
		y = rowY + 4;
...
async function drawCoverPage(doc, title, subtitle, image) {
	const w = doc.internal.pageSize.getWidth();
	const h = doc.internal.pageSize.getHeight();
	doc.setFillColor(BRAND.r, BRAND.g, BRAND.b);
	doc.rect(0, 0, w, h, "F");
	doc.setFillColor(ACCENT.r, ACCENT.g, ACCENT.b);
	doc.rect(0, h * .55, w, 4, "F");
	doc.setFillColor(255, 255, 255);
	doc.rect(0, h * .55 + 4, w, .4, "F");
	const pillW = 70;
	const pillH = 28;
	...
	const logo = await loadImage(csi_logo_default);
	if (logo) {
		...
		try {
			doc.addImage(logo.dataUrl, detectFormat(logo.dataUrl), pillX + (pillW - dw) / 2, pillY + (pillH - dh) / 2, dw, dh);
		} catch {}
	}
	doc.setTextColor(255, 255, 255);
	doc.setFont("helvetica", "bold");
	doc.setFontSize(26);
	doc.text("CSI FANS", w / 2, 68, { align: "center" });
	doc.setFont("helvetica", "normal");
	doc.setFontSize(11);
	doc.setTextColor(200, 220, 235);
	doc.text("Product Catalogue", w / 2, 75, { align: "center" });
	if (image) {
		...
		if (img) {
			...
			try {
				doc.addImage(img.dataUrl, detectFormat(img.dataUrl), (w - dw) / 2, h * .55 + 15, dw, dh);
			} catch {}
		}
	}
	doc.setTextColor(255, 255, 255);
...
async function downloadCategoryCatalogue(cat) {
	const doc = new jsPDF({
		unit: "mm",
		format: "a4"
	});
...
async function downloadProductCatalogue(cat, model) {
	const doc = new jsPDF({
		unit: "mm",
		format: "a4"
	});
...
async function downloadFullCatalogue(cats) {
	const doc = new jsPDF({
		unit: "mm",
		format: "a4"
	});
	await drawCoverPage(doc, "Full Product Catalogue", "Ceiling, pedestal, table, wall and premium fans engineered for Indian homes and offices.", cats[0]?.image);
	for (const cat of cats) {
		...
		if (heroImg) {
			...
			try {
				doc.addImage(heroImg.dataUrl, detectFormat(heroImg.dataUrl), 14 + (boxW - dw) / 2, boxY + (boxH - dh) / 2, dw, dh);
			} catch {}
		}
		for (const model of cat.models) {
			doc.addPage();
...
//#endregion
export { downloadFullCatalogue as n, downloadProductCatalogue as r, downloadCategoryCatalogue as t };
